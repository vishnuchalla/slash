...
def extractProducts(args):
    ...
    link = bool(args.link) if isinstance(args, argparse.Namespace) else bool(args.get('link', True))
    num = int(args.num) if isinstance(args, argparse.Namespace) else int(args.get('num', 3))
    sort = args.sort if isinstance(args, argparse.Namespace) else args.get('sort', "re")
    des = bool(args.des) if isinstance(args, argparse.Namespace) else bool(args.get('des', True))
...
def send_email(results, args):
    email = args.email if isinstance(args, argparse.Namespace) else args.get('email', "")
    link = bool(args.link) if isinstance(args, argparse.Namespace) else bool(args.get('link', True))
    try:
        if email:
